...
model = dict(
    ...
    neck=[dict(
        ...
        dict(type='Inception',in_channel=256,num_levels=6,share=True)
    ],
    rpn_head=dict(
        type='RPNHead',
        in_channels=256,
...
data = dict(
    ...
    train=dict(
        type=dataset_type,
        ann_file=data_root + 'DarkFace_Train_1xmsrcr.json',
        img_prefix=data_root ,
        pipeline=train_pipeline),
    val=dict(
        type=dataset_type,
        ann_file=data_root + 'DarkFace_Val_1xmsrcr.json',
        img_prefix=data_root,
        pipeline=test_pipeline),
    test=dict(
        type='CocoDataset',
        ann_file='/data/Dataset/DarkFace/Test_Images/Test_Final.json',
        img_prefix='/data/Dataset/DarkFace/Test_Images/Track1.2_testing_samples_MSRCR2/',
        pipeline=test_pipeline))
